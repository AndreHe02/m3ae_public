...
def main(argv):
    ...
    def generate_batch(iterator):
        while True:
            batch = next(iterator)
            b = {}
            b["image"] = (batch["observations"]["image"].astype(np.float32)/255.0 - imagenet_mean) / imagenet_std
            b["target_image"] = (batch["goals"]["image"].astype(np.float32)/255.0 - imagenet_mean) / imagenet_std
    ...
    val_data_iterator = prefetch_to_device(generate_batch(val_data_iter), 2, jax_devices)
    ...
    for step, batch in zip(step_counter, data_iterator):
        ...
        if FLAGS.plot_freq > 0 and step % FLAGS.plot_freq == 0:
            ...
            for pref, batch in [('train', train_batch), ('val', val_batch)]:
                ...
                text = text_processor.decode(text_ids)[:n]
        ...
        if FLAGS.save_model_freq > 0 and step % FLAGS.save_model_freq == 0:
            save_data = {
                ...
                #    FLAGS.save_dir, state, step=step,
                #)
